...
const app = getApp();
const dbcol = wx.cloud.database().collection("joinUs2020Fall");
...
Component({
    ...
    data: {
        ...
        departments: [
            '社团财务监督委员会', '办公室', '人力资源部', '社团部', '行政监察部', '公共关系部',
    ...
    methods: {
        ...
        toFormObject(data) {
            ...
            // trim and judge
            const trimArr = [
                ["name", "姓名", /^[\u4e00-\u9fa5·a-zA-Z]{2,10}$/],
                ["studentId", "学号", /^[UMD]?\d{9}$/],
                ["faculty", "专业班级"],
                ["mobile", "手机号码", /^\d{11}$/],
                ["apartmentBuilding", "宿舍地址", /^[1-9]\d?$/]
            ];
            ...
            return {
                apartment: this.data.apartmentArray[this.data.apartmentIndex] + data.apartmentBuilding + "栋",
